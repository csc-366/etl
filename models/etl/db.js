...
export async function ingest(data) {
    ...
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            await query(connection, "SAVEPOINT observation_start");
            ...
            if (!observation.date || !observation.location) {
                ...
                continue;
            } else if (observation.date === 'NaN-NaN-NaN') {
                errors[observation.rowNumber] = 'Invalid Date';
                continue;
            }
            ...
            try {
                ...
                await connection.query("RELEASE SAVEPOINT observation_start");
            }
            catch
                (e) {
                console.error(e.stack);
                await query(connection, 'ROLLBACK TO observation_start');
                ...
                if (e.message === invalidLocation) {
                    errorMessage = "Invalid Location";
                }
                errors[observation.rowNumber] = errorMessage;
            }
        }
        const fatalErrors = Object.entries(errors)
            .filter(([row, msg]) => (/(?:.*)(?:Date|Location)(?:.*)/.test(msg)))
            .reduce((agg, [key, value]) => ({...agg, [key]: value}), {});
        hadFatalError = Object.values(fatalErrors).length > 0;
        if (hadFatalError) {
            await query(connection, "ROLLBACK");
            return {errors: fatalErrors, hadFatalError};
        }
        await query(connection, "COMMIT");
        return {errors, hadFatalError};
    } catch (e) {
        await query(connection, 'ROLLBACK');
        return {errors: {system: e.message}}
    } finally {
        connection.release();
    }
}