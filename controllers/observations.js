...
import { match, retrieveMatches } from "../models/match";
...
export async function all(req, res) {
  ...
  const formattedPendingList = pendingList.map(observation =>
...
export async function validateObservation(req, res) {
  ...
  if (!isValidDate(date)) {
    sendError(res, 400, [
      "Invalid observation: Supplied date is either in the future, or before 2017"
    ]);
    return;
  }
  ...
  console.log(sealScores);

  const matchingSeals = await retrieveMatches(sealScores);

  sendData(res, matchingSeals);
}
...
export async function submitObservation(req, res) {
  ...
  try {
    ...
    if (body.tags && body.tags.length) {
      ...
      sendError(res, 400, [
        "Could not add seal to database. No marks or tags found in observation"
      ]);
    }
...
const mapObservation = observation => {
  const formattedObservation = Object.entries(observation).reduce(
    (agg, [key, value]) => {
      switch (key) {
        case "Age":
        case "Comments":
...
async function respondWithSealMatches(res, tagNums, markNums) {
  ...
  if (tagNums.length) {
    ...
    if (seal) {
      ...
      sendData(res, {
        status: "OK",
        response: { seal: null, sealObservations: [] }
      });
    }
    ...
    sendError(res, 400, [
      "Could not add seal to database. No marks or tags" +
...
async function respondWithPotentialMatches(res, tagNums, markNums) {
  ...
  if (tagNums.length) {
    ...
    potentialSeals = await getSealsFromPartialMark(
      markNums[0],
      markNums.season
    );
...
async function invalidNewIdentifiers(req, res, completeTags, completeMarks) {
  for (let i = 0; i < completeTags.length; i++) {
    ...
    if (tag && req.body.tags[i].isNew) {
      sendError(res, 400, [
        "A matching tag that is listed as new already exists in the database."
      ]);
      return true;
    }
  }
  for (let i = 0; i < completeMarks.length; i++) {
    ...
    if (mark && req.body.marks[i].isNew) {
      sendError(res, 400, [
        "A matching mark that is listed as new already exists in the database."
      ]);
      return true;
    }
  }
